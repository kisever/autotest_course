...
def everything_for_your_cat(cats_data):
    """
    Из полученного списка с данными составляет строку
    :param cats_data: список кортежей, содержащий: Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя.
    ...
    for i in range(len(cats_data)):
        fio = ' '.join(cats_data[i][2:])  # фамилия и имя хозяина из текущей записи списка
        cat = ', '.join(map(str, cats_data[i][:2]))  # кличка и возраст кота
        dic_cats[fio] = f'{dic_cats[fio]}; {cat}' if dic_cats.get(fio) else cat
    our_str = ''
    # из полученного словаря формируем строку
    for i in range(len(dic_cats)):
        our_str = f"{': '.join(dic_cats.popitem())}\n{our_str}"
    return our_str
